...
if (isset($_POST['update_introduction'])) {
	...
	$profile_query = $con->prepare("SELECT users FROM profile_user WHERE username=?");
	$profile_query->bind_param("s", $userLoggedIn);
	$profile_query->execute();

	$result = $profile_check->get_result();
		...
		if (in_array($fileActualExt, $allowed)) {
			if ($fileError === 0) {
				if ($fileSize < 1000000) {
					...
					$insert_pic_query = $con->prepare("UPDATE users SET profile_pic=? WHERE users=?");
		...
		if (in_array($fileActualExt, $allowed)) {
			if ($fileError === 0) {
				if ($fileSize < 1000000) {
					...
					$insert_pic_query = $con->prepare("UPDATE users SET cover_pic=? WHERE users=?");
					$insert_pic_query->bind_param("ss", $directory, $userLoggedIn);
					$insert_pic_query->execute();
